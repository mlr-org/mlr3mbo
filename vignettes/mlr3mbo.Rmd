...
To write an own `loop_function`, users can get inspiration from the readily available ones, e.g., `r ref("mlr3mbo::bayesopt_ego", "bayesopt_ego")` which performs sequential single-objective optimization:
...
* A `r ref("mlr3learners::LearnerRegrRanger", "Random forest")` for higher dimensional mixed (and / or hierarchical) search spaces
