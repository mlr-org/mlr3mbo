...
fun = function(xdt) {
  ...
  b = 0.2
  c = 2 * pi
  y = -a * exp(-b * sqrt((1 / 2) * rowSums(xdt^2))) - exp((1 / 2) * rowSums(cos(c * xdt))) + a + exp(1)
  data.table(y = y)
}
...
bo_rf_ei = OptimizerMbo$new(
  acq_function = default_acqfun(instance, default_surrogate(instance, learner = lrn("regr.ranger")))
)
