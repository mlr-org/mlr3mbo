bayesopt_trafbo = function(instance, init_design_size = NULL) {
  ...
  repeat {
    if (runif(1) <= 0.2) {
      ...
      ctrl = ctree_control(minbucket = 5L, alpha = 0.05, maxdepth = 10L)
      ...
      learner = as_learner(po("imputeoor", multiplier = 10L) %>>% lrn("regr.ranger", splitrule = "extratrees", num.trees = 1000L, min.node.size = 1L))
      learner$train(task)
      ...
      if (runif(1) <= 0.5) {
        ...
        opt("random_search", batch_size = 1000L)$optimize(f_instance)
      } else {
        f_instance = make_greedy_f_instance(tmp_ss, learner = learner, n_evals = 20000L)
