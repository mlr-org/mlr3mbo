  ...
  private = list(
    ...
    .predict = function(task) {
      ...
      if (isTRUE(pv$se.method == "simple")) {
        # the variance is only correct if min.node.size = 1 and therefore sigma^2_b is always 0 or rather undefined (as
        # there is always only a single observation in each leaf)
        sigma_min = 0.01
        ...
        tmp = sigma_min ^ 2 + prediction$predictions ^ 2
        variance = rowMeans(tmp) - response ^ 2
