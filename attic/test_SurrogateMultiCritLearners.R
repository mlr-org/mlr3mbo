test_that("SurrogateMultiCritLearners API works", {
  ...
  expect_data_table(pred[[1L]], col.names = "named", nrows = 5, ncols = 2, any.missing = FALSE)
  expect_data_table(pred[[2L]], col.names = "named", nrows = 5, ncols = 2, any.missing = FALSE)
...
test_that("insample_perf", {
  ...
  expect_error(
    {
      surrogate$insample_perf = c(0, 0)
    },
    regexp = "Field/Binding is read-only."
  )
  expect_error(
    {
      surrogate$assert_insample_perf = 0
    },
    regexp = "Field/Binding is read-only."
  )
  design = generate_design_lhs(PS_1D, 4)$data
  ...
  expect_error(
    surrogate_constant$update(xydt = xydt, y_cols = c("y1", "y2")),
    regexp = "Current insample performance of the Surrogate Model does not meet the performance threshold"
  )
