test_that("SurrogateLearners API works", {
  inst = MAKE_INST(OBJ_1D_2, PS_1D, trm("evals", n_evals = 5L))
  design = MAKE_DESIGN(inst)
  inst$eval_batch(design)
  surrogate = SurrogateLearners$new(learners = list(REGR_KM_DETERM, REGR_KM_DETERM$clone(deep = TRUE)), archive = inst$archive)
...
test_that("predict_types are recognized", {
  ...
  surrogate = SurrogateLearners$new(learner = list(REGR_KM_DETERM, lrn("regr.rpart")), archive = inst$archive)
  surrogate$update()
...
test_that("param_set", {
  inst = MAKE_INST(OBJ_1D_2, PS_1D, trm("evals", n_evals = 5L))
  surrogate = SurrogateLearners$new(learner = list(REGR_KM_DETERM, REGR_KM_DETERM$clone(deep = TRUE)), archive = inst$archive)
...
test_that("insample_perf", {
  ...
  surrogate_constant = SurrogateLearners$new(learner = list(lrn("regr.featureless"), lrn("regr.featureless")), archive = inst$archive)
...
test_that("unique in memory", {
  ...
  expect_error(SurrogateLearners$new(learners = list(learner, learner)), "Redundant Learners")
})

test_that("deep clone", {
  inst = MAKE_INST(OBJ_1D_2, PS_1D, trm("evals", n_evals = 5L))
  learner = REGR_KM_DETERM
  surrogate1 = SurrogateLearners$new(learners = list(learner, learner$clone(deep = TRUE)), archive = inst$archive)
