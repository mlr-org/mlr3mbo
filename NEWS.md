...
* feat: New loop function `bayesopt_thompson` for search spaces that are partitioned into homogeneous subspaces along a categorical parameter, e.g., the parameter that selects the learner in combined algorithm selection and hyperparameter optimization. In each iteration, one subspace is sampled with Beta-Bernoulli Thompson sampling, counting the evaluations in the top `top_quantile` fraction of the archive as successes, and the surrogate, the acquisition function, and the acquisition function optimizer only see the evaluations and the parameters of that subspace, which removes most of the dependencies of the search space from the modeling problem.
* feat: `OptimizerADBOThompson` and `TunerADBOThompson` interleave the initial designs of the subspaces that share a `mirai` compute profile round-robin instead of queuing them subspace by subspace, so that the subspaces are covered evenly when the optimization terminates during the initial design.
* feat: New `OptimizerADBOThompson` and `TunerADBOThompson` run ADBO on a search space that is partitioned into homogeneous subspaces, typically one subspace per learner. Several subspaces may share a `mirai` compute profile, e.g., `subspace_profiles = c(tabpfn = "gpu", ranger = "cpu", xgboost = "cpu")`, and a worker samples one of the subspaces of its profile via Beta-Bernoulli Thompson sampling in each iteration. With one subspace per profile, the optimizer reduces to `OptimizerADBOSubspaces`.
...
* feat: New `partition_search_space()` splits a hierarchical search space along the levels of a categorical parameter, e.g., the branching parameter of a `mlr3pipelines::Graph`, into the subspaces consumed by `OptimizerADBOSubspaces`, `OptimizerADBOThompson`, and `bayesopt_thompson`. By default, every level becomes a subspace of its own.
