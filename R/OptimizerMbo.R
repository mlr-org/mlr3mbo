  ...
  public = list(
    ...
    initialize = function(
      ...
      if (!is.null(self$loop_function)) {
        assert_subset(
          names(args),
          choices = setdiff(
            names(formals(self$loop_function)),
    ...
    optimize = function(inst) {
      ...
      if (is.null(self$acq_function)) {
        # acq_optimizer$acq_function has precedence
  ...
  private = list(
    ...
    .optimize = function(inst) {
      if (!is.null(self$param_set$values$initial_design)) {
        ...
        inst$archive$add_evals(
          xdt = xydt[, inst$archive$cols_x, with = FALSE],
