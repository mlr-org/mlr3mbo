...
bayesopt_mego = function(
  ...
  acq_function = ObjectiveRFunDt$new(
    fun = function(xdt) {
      setNames(map_dtc(acq_functions, function(acqf) acqf$eval_dt(xdt)), nm = acq_function_codomain$ids())
    },
    domain = instance$search_space$clone(deep = TRUE),
    codomain = acq_function_codomain
  )
