...
num.trees = 2
mytask = TaskRegr$new(id = "mytask", backend = dd, target = "y")
y = mytask$truth()

mylrn = lrn("regr.ranger_mbo", num.trees = num.trees, predict_type = "se", se.method = "law_of_total_variance")
mylrn$train(mytask)

zz = compute_mu_sigma2(mylrn$model$model, mytask)
ms = zz$mu_sigma2_per_node_per_tree
termtab = zz$prediction_nodes$predictions
print(ms)
print(termtab)

# ms2 = mylrn$model$mu_sigma
# print(ms2)
# sv1 = simple_var(mylrn, dd, ms1)
# print(sv1)
# sv2 = mylrn$predict(mytask)
# print(sv2)

ltv1 = ltv(mylrn, dd, ms)
print(ltv1)
ltv2 = mylrn$predict(mytask)
print(ltv2)
