...
[Surrogate](https://mlr3mbo.mlr-org.com/reference/Surrogate.md), an
...
[Surrogate](https://mlr3mbo.mlr-org.com/reference/Surrogate.md) is
...
inspect the
[Surrogate](https://mlr3mbo.mlr-org.com/reference/Surrogate.md) after
optimization if it is to be used, e.g., for visualization purposes to
make sure that it has been properly updated on all available data. If
this final update of the
[Surrogate](https://mlr3mbo.mlr-org.com/reference/Surrogate.md) could
not be performed successfully, a warning will be logged.
...
- `Mlr3ErrorMboSurrogateUpdate`:
  ...
  Raised by
  [Surrogate](https://mlr3mbo.mlr-org.com/reference/Surrogate.md)`$update()`
...
- `Mlr3ErrorMboAcqOptimizer`:
  ...
  Raised by
  [AcqOptimizer](https://mlr3mbo.mlr-org.com/reference/AcqOptimizer.md)`$optimize()`
...
populated, this
[Surrogate](https://mlr3mbo.mlr-org.com/reference/Surrogate.md) is used.
Otherwise, `default_surrogate(instance)` is used. If `acq_function` is
`NULL` and the `acq_optimizer$acq_function` field is populated, this
[AcqFunction](https://mlr3mbo.mlr-org.com/reference/AcqFunction.md) is
used (and therefore its `$surrogate` if populated; see above). Otherwise
`default_acqfunction(instance)` is used. If `acq_optimizer` is `NULL`,
`default_acqoptimizer(instance)` is used.
...
#> WARN  [17:50:30.182] [bbotk] Could not update the surrogate a final time after the optimization process has terminated.
#>            x  x_domain        y1       y2
#>        <num>    <list>     <num>    <num>
