...
// Helper: compute mean and variance using Welford's algorithm
...
SEXP c_mu_sigma2_per_node_per_tree(SEXP obs_node_table, SEXP y) {
  ...
  // This allocates the max amount memory possibly needed; could be optimized.
...
SEXP c_ltv_var(SEXP predictions, SEXP mu_list) {
  if (!isInteger(predictions) || !isMatrix(predictions)) {
    error("predictions must be an integer matrix");
  }
  if (!isNewList(mu_list)) {
  ...
  for (int i = 0; i < n_obs; ++i) {
    ...
    for (int j = 0; j < n_trees; ++j) {
      ...
      double delta = mu - mean_mu;
      mean_mu += delta / (j + 1);

      double delta2 = (mu * mu + sigma2) - mean_musq_plus_sigma2;
      mean_musq_plus_sigma2 += delta2 / (j + 1);
    }
    resp[i] = mean_mu;
    double se2 = mean_musq_plus_sigma2 - mean_mu * mean_mu;
    serr[i] = sqrt(se2 > 0.0 ? se2 : 0.0);
  }
